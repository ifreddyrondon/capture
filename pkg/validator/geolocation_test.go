...
func TestValidateGeolocationOK(t *testing.T) {
	...
	tt := []struct {
		name     string
		body     string
		expected validator.GeoLocation
	}{
		{
		...
		},
		{
			...
			body:     `{"lat":75,"lng":180}`,
			expected: validator.GeoLocation{LAT: f2P(75), LNG: f2P(180)},
		},
		{
		...
		},
		{
			...
			expected: validator.GeoLocation{LAT: f2P(75), LNG: f2P(180), Elevation: f2P(1)},
		},
		{
		...
		},
		{
			...
			expected: validator.GeoLocation{LAT: f2P(90), LNG: f2P(-147.45)},
		},
		{
			name:     "decode point with lat, lng for lower limits",
			body:     `{"lat":-90.00000,"lng":-180.0000}`,
			expected: validator.GeoLocation{LAT: f2P(-90.00000), LNG: f2P(-180.0000)},
		},
		{
			name:     "decode point with lat, lng with decimals",
			body:     `{"lat":77.11112223331,"lng":149.99999999}`,
			expected: validator.GeoLocation{LAT: f2P(77.11112223331), LNG: f2P(149.99999999)},
		},
	}
